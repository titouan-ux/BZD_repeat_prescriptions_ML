...
results = pyreadr.read_r("/rds/general/project/hda_24-25/live/TDS/Group03/Datasets/imputed_train_male_bio_X.rds")
X_train = list(results.values())[0]

results = pyreadr.read_r("/rds/general/project/hda_24-25/live/TDS/Group03/Datasets/Y_train_male_bio.rds")
Y_train = list(results.values())[0]
Y_train = Y_train.values.ravel()  # Converts (n,1) to (n,)

results = pyreadr.read_r("/rds/general/project/hda_24-25/live/TDS/Group03/Datasets/Y_test_male_bio.rds")
Y_test = list(results.values())[0]
Y_test = Y_test.values.ravel()  # Converts (n,1) to (n,)

results = pyreadr.read_r("/rds/general/project/hda_24-25/live/TDS/Group03/Datasets/imputed_test_male_bio_X.rds")
X_test = list(results.values())[0]

sex="male"
results = pyreadr.read_r("/rds/general/project/hda_24-25/live/TDS/Group03/list_bio_var_global.rds")
global_list_bio = list(results.values())[0]

print(len(X_train.columns))
X_train = X_train.drop(columns=set(global_list_bio.iloc[:,0]) & set(X_train.columns))
X_test = X_test.drop(columns=set(global_list_bio.iloc[:,0]) & set(X_test.columns))
save_path ="/rds/general/project/hda_24-25/live/TDS/Group03/RF/biomarkers_and_all/no_biomarkers"
...
with open(f"{save_path}/best_params_OOB_{sex}.json", "w") as f:
...
plt.title("ROC Curve")
plt.legend()
plt.savefig(f"{save_path}/roc_curve_RF_{sex}.png", dpi=300, bbox_inches="tight")

# Save FPR and TPR as a CSV file
np.savetxt(f"{save_path}/roc_curve_RF_{sex}.txt",
           np.column_stack((fpr, tpr)), delimiter=",", header="FPR,TPR", comments="")

# Save AUC separately
with open(f"{save_path}/auc_RF_{sex}.txt", "w") as f:
    f.write(f"AUC: {auc}\n")
...
with open(f"{save_path}/importances_var_{sex}.json", "w") as f:
...
plt.savefig(f"{save_path}/variable_importance_plot_{sex}.png", dpi=300, bbox_inches="tight")
...
# 
# fig.tight_layout()
# plt.savefig(f"{save_path}/permutation_importance_plot_{sex}.png", dpi=300, bbox_inches="tight")
# 
...
plt.savefig(f"{save_path}/shap_bar_plot_{sex}.png", dpi=300, bbox_inches="tight")
...
plt.savefig(f"{save_path}/shap_density_plot_{sex}.png", dpi=300, bbox_inches="tight")
